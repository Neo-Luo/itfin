...
def entityportrait():
	if session:
		...
		role_id = ""
		uid = ""
	return render_template('entityPortrait/entity_portrait.html',username=username,role_id=role_id,uid=uid)
...
def platform():
	result = get_platform(TABLE_MONITOR)
	return json.dumps(result,ensure_ascii=False)

@entityPortrait.route('/company/',methods=['POST','GET'])
def company():
	result = get_company(TABLE_MONITOR)
	return json.dumps(result,ensure_ascii=False)

@entityPortrait.route('/project/',methods=['POST','GET'])
def project():
	result = get_project(TABLE_MONITOR)
	return json.dumps(result,ensure_ascii=False)
