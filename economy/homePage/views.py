...
def warn_count():
	result = h_getWarnCount(TABLE_MONITOR, RISK_LEVEL, ILLEGAL_SCORE)
	return json.dumps(result,ensure_ascii=False)

@homePage.route('/cityRank/')
def city_rank():
	province = request.args.get('province','')
	result = get_city_rank(TABLE_MONITOR,TABLE_GONGSHANG,province, RISK_LEVEL, ILLEGAL_SCORE)#读取预存储数据需要注释掉这句
	#result = get_prepared_city_rank(province)    #建好表计算出数据后把这句恢复了就可以读取预存储数据了
	return json.dumps(result,ensure_ascii=False)

@homePage.route('/provinceRank/')
def province_rank():
	result = get_province_rank(TABLE_MONITOR,TABLE_GONGSHANG, RISK_LEVEL, ILLEGAL_SCORE)
...
def hot_spot():
	list = get(TABLE_ENTITY_LIST,TABLE_PLAT_DETAIL,TABLE_GONGSHANG,0,10000,0,'all','all',TABLE_INDEX_QUANTILE,TABLE_GUARANTEE_PROMISE)['data'][0:1000]
	entity_list = []
	for dict in list:
